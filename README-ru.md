...
(примечание редактора: читая эти документы И.Паскаля, я так и не понял чем отличается latched от base. Объяснение я нашел позже на странице https://wiki.archlinux.org/title/X_keyboard_extension : SetMods makes a regular "on while pressed" modifier key. LockMods makes an "on/off" switch like CapsLock or NumLock. LatchMods means "on until next keypress" aka sticky modifier)
...
| | *   если этих модификаторов нет в **locked modifiers**, то добавляет их туда, в противном случае наоборот - убирает | *   **locked modifiers** не меняется. |
...
А вот то, как сделать переключение между тремя (и больше) группами более приятным, мы рассмотрим в следующем примере -
...
type "THREE_LEVEL" { 
    ...
    level_name[Level1] = "Base"; 
    level_name[Level2] = "Shift"; l
    evel_name[Level3] = "Level3"; 
};
```
...
Вспомним еще раз - в чем проблема. В том, что основной переключатель должен переходить из одного состояния (текущая группа - Group1), в два разных, в зависимости от состояния какой-то другой клавиши.

Но ведь можно "запомнить" состояние дополнительного переключателя не только с помощью модификатора.
...
Тогда основной переключатель, чтобы перейти из "лат" в одну из "рус" раскладок должен просто обнулить **locked group**. А для того, чтобы вернуться обратно в "лат", надо в **locked group** записать "добавку", достаточно большую, чтобы "метод выравнивания" "завернул" значение обратно к "лат" раскладке.
...
Прежде чем переходить к написанию конфигурации клавиш, давайте проясним вопрос: что значит "обнулить" переменную с номером группы, и - какие "добавки" нам понадобятся.
...
Обратите внимание, что в названиях присутствуют - **First_Group, Last_Group, Next_Group** и **pv_Group**.
...
```
xkb_compat { 
    interpt ISO_First_Group_Lock { 
        action=LockGroup(group=1);
    }; 
    interpt ISO_Next_Group_Lock { 
        action=LockGroup(group=2);
    }; 
    interpt ISO_Last_Group_Lock { 
        action=LockGroup(group=3);
    }; 
    interpt ISO_Group_Lock { 
